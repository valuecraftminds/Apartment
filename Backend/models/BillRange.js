...
class BillRange{
    static async create(billRangeData){
        const {company_id,bill_id, fromRange,toRange} = billRangeData;
        const id = uuidv4().replace(/-/g, '').substring(0, 10);

        const[result] = await pool.execute(
            'INSERT INTO billrange(id, company_id, bill_id, fromRange, toRange, is_active) values (?, ?, ?, ?, ?, 1) ',
            [id, company_id, bill_id, fromRange, toRange]
        );
    ...
    static async update(id, billRangeData) {
        const {fromRange,toRange } = billRangeData;
        await pool.execute(
            'UPDATE billrange SET fromRange = ?,toRange = ? WHERE id = ?',
            [ fromRange,toRange,id]
        );
        return { id, ...billRangeData };
    }
