...
export default function ViewComplaint() {
  ...
  const [filters, setFilters] = useState({
    status: 'all',
    apartment_id: 'all',
    category_id: 'all'  // Changed from category to category_id to match database
  });
    ...
    try {
      ...
      const res = await api.get('/complaints', {
        ...
        params: {
          ...
          apartment_id: filters.apartment_id !== 'all' ? filters.apartment_id : undefined,
  ...
  useEffect(() => {
    if (auth.accessToken) {
      loadApartments();
      fetchCategories();  // Call fetchCategories here
      fetchComplaints();
    }
  }, [auth.accessToken]);
  ...
  const getCategoryColor = (category) => {
    ...
    // Find the category from the categories array to get the category_name
    const categoryObj = categories.find(cat => cat.id === category.category_id);
    const categoryName = categoryObj ? categoryObj.name : category.name || 'General';
  ...
  const getCategoryName = (category) => {
    ...
    // If category is an object with category_id
    if (category.category_id) {
      const categoryObj = categories.find(cat => cat.id === category.category_id);
      return categoryObj ? categoryObj.name : 'General';
    }
    
    // If category is just a string
    const categoryObj = categories.find(cat => cat.name === category);
    return categoryObj ? categoryObj.name : category || 'General';
  };
  ...
  const handleDelete = (complaint) => {
      ...
      try {
        ...
        await api.delete(`/complaints/${deletingComplaint.id}`);
        ...
        <div className='flex-1 overflow-y-auto p-4 md:p-6'>
          <div className='mx-auto max-w-7xl'>
            ...
            <div className='flex flex-col sm:flex-row sm:items-center justify-between bg-white dark:bg-gray-800 rounded-xl md:rounded-2xl p-4 mb-4 md:mb-6 gap-4'>
              <div className='flex items-center'>
                <AlertCircle size={isMobile ? 28 : 32} className='text-purple-600 dark:text-purple-400 mr-3 flex-shrink-0'/>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-4 md:mb-6">
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-lg md:text-2xl font-bold text-gray-800 dark:text-white mt-1">
                      {statistics.total}
                    </p>
                  </div>
                  <div className="p-1.5 md:p-2 bg-purple-100 dark:bg-purple-900 rounded-lg">
                    <FileText size={isMobile ? 16 : 20} className="text-purple-600 dark:text-purple-400" />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-lg md:text-2xl font-bold text-yellow-600 dark:text-yellow-400 mt-1">
                      {statistics.pending}
                    </p>
                  </div>
                  <div className="p-1.5 md:p-2 bg-yellow-100 dark:bg-yellow-900 rounded-lg">
                    <Clock size={isMobile ? 16 : 20} className="text-yellow-600 dark:text-yellow-400" />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">In Progress</p>
                  ...
                  <div className="p-1.5 md:p-2 bg-blue-100 dark:bg-blue-900 rounded-lg">
                    <Wrench size={isMobile ? 16 : 20} className="text-blue-600 dark:text-blue-400" />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="p-1.5 md:p-2 bg-green-100 dark:bg-green-900 rounded-lg">
                    <CheckCircle size={isMobile ? 16 : 20} className="text-green-600 dark:text-green-400" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 mb-4 md:mb-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4">
                <h3 className="text-base md:text-lg font-semibold text-gray-800 dark:text-white mb-2 md:mb-0">
                  Filter & Search Complaints
                </h3>
                <div className="flex items-center space-x-2">
                  <Filter size={isMobile ? 16 : 18} className="text-gray-500 dark:text-gray-400" />
              ...
              <div className="relative mb-4">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={isMobile ? 16 : 20} />
                <input
                  type="text"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  placeholder="Search complaints..."
                  className="w-full pl-10 pr-4 py-2 text-sm md:text-base border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:text-white"
                />
              </div>
              ...
              {isMobile && (
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-2 mb-3 bg-gray-100 dark:bg-gray-700 rounded-lg"
                >
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                    Filters {showFilters ? <ChevronUp size={16} className="inline ml-1" /> : <ChevronDown size={16} className="inline ml-1" />}
                  </span>
                  <span className="text-xs text-gray-500 dark:text-gray-400">
                    {filters.status !== 'all' || filters.apartment_id !== 'all' || filters.category_id !== 'all' ? 'Active' : 'None'}
                  </span>
                </button>
              )}
              ...
              <div className={`${isMobile && !showFilters ? 'hidden' : 'block'}`}>
                <div className="space-y-3 md:space-y-0 md:grid md:grid-cols-3 gap-3 md:gap-4">
                  {/* Status Filter */}
                  <div>
                    <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
                      Status
                ...
                <div className="mt-4 md:mt-3">
                  <button
                    onClick={clearFilters}
                    className="w-full px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors text-sm md:text-base"
                  >
                    Clear Filters
                  </button>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
              {filteredComplaints.length === 0 ? (
                <div className="text-center py-8 md:py-12">
                  <AlertCircle size={isMobile ? 32 : 48} className="mx-auto text-gray-400 mb-3 md:mb-4" />
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <h4 className="text-sm font-semibold text-gray-800 dark:text-white mb-1 line-clamp-1">
                            {complaint.title}
                          </h4>
                          <div className="flex items-center text-xs text-gray-500 dark:text-gray-400 mb-2">
                            <Building2 size={12} className="mr-1" />
                            <span className="truncate">{complaint.apartment_name || 'N/A'}</span>
                          </div>
                        </div>
                        <div className="ml-2">
                      ...
                      <div className="flex justify-between items-center text-xs text-gray-600 dark:text-gray-400 mb-2">
                        <div className="flex items-center">
                          ...
                          <span className="truncate">{complaint.houseowner_name || 'Unknown'}</span>
                        </div>
                        <div className="text-right">
                          <span>{formatShortDate(complaint.created_at)}</span>
                        </div>
                      </div>

                      {/* Category */}
                      <div className="mb-2">
                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getCategoryColor(complaint)}`}>
                      ...
                      {expandedComplaint === complaint.id && (
                        <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600 space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            <div>
                              <p className="font-medium text-gray-700 dark:text-gray-300">Floor</p>
                              <p className="text-gray-600 dark:text-gray-400">{complaint.floor_number || 'N/A'}</p>
                            </div>
                            <div>
                              <p className="font-medium text-gray-700 dark:text-gray-300">House</p>
                              <p className="text-gray-600 dark:text-gray-400">{complaint.house_number || 'N/A'}</p>
                            </div>
                          </div>
                          ...
                          <div className="space-y-1 text-xs">
                            <div className="flex justify-between">
                              ...
                              <span className="text-gray-600 dark:text-gray-400">{formatDate(complaint.created_at)}</span>
                            </div>
                            {complaint.resolved_at && (
                              <div className="flex justify-between">
                                <span className="text-gray-700 dark:text-gray-300">Resolved:</span>
                                <span className="text-gray-600 dark:text-gray-400">{formatDate(complaint.resolved_at)}</span>
                              </div>
                            )}
                            <div className="flex justify-between">
                              ...
                              <span className="text-gray-600 dark:text-gray-400">
                                {complaint.resolved_at 
                                  ? calculateTimeDifference(complaint.created_at, complaint.resolved_at)
                                  : calculateTimeDifference(complaint.created_at, 'now')}
                              </span>
                            </div>
                            {complaint.total_work_time > 0 && (
                              <div className="flex justify-between items-center text-green-600 dark:text-green-400">
                                ...
                                <span className="flex items-center">
                                  <Timer size={12} className="mr-1" />
                                  {formatWorkTime(complaint.total_work_time)}
                                </span>
                              </div>
                            )}
                          </div>
