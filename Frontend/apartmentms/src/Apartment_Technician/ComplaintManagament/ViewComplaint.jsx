                            ...
                            <div className="space-y-1">
                              {complaint.resolved_at ? (
                                <>
                                  ...
                                  <div className="text-xs text-gray-500 dark:text-gray-400">
                                    Time to resolve: {calculateTimeDifference(complaint.created_at, complaint.resolved_at)}
                                  </div>
                                  {complaint.total_work_time && complaint.total_work_time > 0 && (
                                    <div className="text-xs text-green-600 dark:text-green-400">
                                      Work Time: {formatWorkTime(complaint.total_work_time)}
                                    </div>
                                  )}
                                </>
                              ) : complaint.status === 'In Progress' && complaint.assigned_at ? (
                                <div className="text-sm text-blue-600 dark:text-blue-400">
                                  In progress for: {calculateTimeDifference(complaint.assigned_at, new Date())}
                                </div>
                              ) : (
                          ...
                          <td className="px-4 py-4">
                            <div className="flex space-x-2">
                              <button
                                ...
                                className="p-2 text-purple-600 hover:text-purple-800 dark:text-purple-400 dark:hover:text-purple-300 transition-colors hover:bg-purple-50 dark:hover:bg-purple-900/30 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
                                title={complaint.assigned_to ? "Already assigned" : "Assign to Technician"}
                              >
                                <Plus size={16} />
                              </button>
                              {complaint.status === 'In Progress' && (
