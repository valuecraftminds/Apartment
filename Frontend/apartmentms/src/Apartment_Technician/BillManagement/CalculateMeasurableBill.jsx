...
//     const manualData = location.state?.manualData || null;
//     const billData = location.state?.billData || manualData?.bill || null;
    
//     // Determine source and extract data accordingly
//     const dataSource = location.state?.source || (scannedData ? 'qr' : 'manual');
//     const houseData = dataSource === 'qr' ? scannedData : manualData?.house;
...
//             toast.error('No house data provided');
//             navigate('/measurable-bills');
//             return;
//         }
...
//             billData
//         });
//     }, [bill_id, scannedData, manualData, navigate]);
...
//             // Pattern 1: Query parameters with month name
//             try {
...
//             } catch (err1) {
//                 apiError = err1;
//                 // Pattern 2: Query parameters with month number
//                 try {
//                     res = await api.get(`/billprice`, {
//                         params: {
//                             bill_id: bill_id,
...
//         // Get house ID based on source
//         if (dataSource === 'qr') {
//             houseId = scannedData?.hse;
//         } else if (dataSource === 'manual') {
//             houseId = manualData?.house?.id || manualData?.house?.houseId;
//         }
...
//         if (dataSource === 'qr') {
//             // QR data structure
//             houseId = scannedData?.hse || scannedData?.h_id;
//             apartmentId = scannedData?.apt || scannedData?.apartment_db_id;
//             floorId = scannedData?.flr || scannedData?.floor_db_id;
//         } else if (dataSource === 'manual') {
//             // Manual data structure
//             houseId = manualData?.house?.id || manualData?.house?.houseId;
//             apartmentId = manualData?.house?.apartment_id;
//             floorId = manualData?.house?.floor_id;
//         }
        
//         console.log("🔍 DEBUG - Extracted IDs for", dataSource, ":", { 
//             houseId, 
//             apartmentId,
//             floorId
//         });
...
//                     month: new Date().getMonth() + 1,
//                     year: new Date().getFullYear(),
//                     previous_reading: formData.current_reading,
//                     current_reading: '',
//                     used_units: '',
//                     total_amount: ''
//                 });
...
//                         navigate(-1); // Go back to manual selection
//                     } else {
//                         navigate(`/house-bill-meter/${bill_id}`); // Go to QR scanner home
//                     }
//                 }, 2000);
...
//     }, [bill_id, scannedData, manualData]);
...
//     // Function to get display values for house information
//     const getHouseDisplayInfo = () => {
//         if (dataSource === 'qr') {
//             return {
//                 houseId: scannedData?.hse || scannedData?.house_id,
//                 apartment: scannedData?.apt || scannedData?.apartment,
//                 floor: scannedData?.flr || scannedData?.floor
//             };
//         } else if (dataSource === 'manual') {
//             return {
//                 houseId: manualData?.house?.house_name || manualData?.house?.id,
//                 apartment: manualData?.house?.apartment_name || manualData?.house?.apartment_id,
//                 floor: manualData?.house?.floor_name || manualData?.house?.floor_id
//             };
//         }
//         return { houseId: 'N/A', apartment: 'N/A', floor: 'N/A' };
//     };

//     const houseDisplayInfo = getHouseDisplayInfo();
...
//                                     </svg>
//                                     Back to {dataSource === 'manual' ? 'My Bills' : 'Measurable Bills'}
//                                 </button>
//                                 <h1 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">
//                                     Calculate Bill: {billData?.bill_name || 'Measurable Bill'}
//                                 </h1>
//                                 <p className="text-gray-600 dark:text-gray-400 mt-1 text-sm md:text-base">
//                                     {dataSource === 'qr' ? 'QR Scanned House' : 'Manually Selected House'}
//                                 </p>
//                             </div>
//                             <div className="flex items-center gap-2">
//                                 <span className={`px-3 py-1 rounded-full text-xs ${dataSource === 'qr' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
//                                     {dataSource === 'qr' ? '📱 QR Scan' : '✍️ Manual Entry'}
//                                 </span>
//                                 <button
//                                     onClick={() => {
//                                         loadBillRanges();
...
//                                                 type="button"
//                                                 onClick={() => navigate(
//                                                     // dataSource === 'manual' ? '/my-bills' : '/measurable-bills'
//                                                     `/house-bill-meter/${bill_id}`
//                                                 )}
...
//     // NEW: Get houseDetails from MeasurableBills.jsx
//     const houseDetails = location.state?.houseData || null;
    
//     // Determine source and extract data accordingly
//     const dataSource = location.state?.source || (scannedData ? 'qr' : 'manual');
    
//     console.log("🔍 Data Source:", dataSource);
//     console.log("🔍 Scanned Data (simplified):", scannedData);
//     console.log("🔍 House Details from parent:", houseDetails);
//     console.log("🔍 Bill Data:", billData);
...
//         // Check if we have the necessary data for QR source
//         if (dataSource === 'qr' && (!scannedData || !houseDetails)) {
//             toast.error('House data is incomplete. Please scan again.');
//             navigate(`/measurable-bills/${bill_id}`);
//             return;
//         }
        
//         // Check if we have the necessary data for manual source
...
//             billData
//         });
//     }, [bill_id, scannedData, manualData, houseDetails, dataSource, navigate]);
...
//         // Get house ID based on source
//         if (dataSource === 'qr') {
//             // NEW: Use the house ID from scanned QR data
...
//         // Extract data based on source
//         if (dataSource === 'qr') {
//             // NEW: Use the simplified data structure
//             // Scanned data contains just houseId
//             houseId = scannedData?.houseId || scannedData?.hse;
            
//             // Get apartment and floor IDs from houseDetails (passed from MeasurableBills.jsx)
//             apartmentId = houseDetails?.apartment_id;
//             floorId = houseDetails?.floor_id;
            
//             console.log("QR Data - houseId from QR:", houseId);
//             console.log("QR Data - apartmentId from houseDetails:", apartmentId);
//             console.log("QR Data - floorId from houseDetails:", floorId);
            
//         } else if (dataSource === 'manual') {
//             // Manual data structure (unchanged)
//             houseId = manualData?.house?.id || manualData?.house?.houseId;
//             apartmentId = manualData?.house?.apartment_id;
//             floorId = manualData?.house?.floor_id;
//         }
...
//                 // Navigate back appropriately
//                 setTimeout(() => {
//                     navigate(`/house-bill-meter/${bill_id}`); // Go back to QR scanner
//                 }, 2000);
...
//     const getHouseDisplayInfo = () => {
//         if (dataSource === 'qr') {
//             // NEW: Use houseDetails for QR source
//             return {
//                 houseId: houseDetails?.house_id || scannedData?.houseId,
//                 apartment: houseDetails?.apartment?.name || 'N/A',
//                 floor: houseDetails?.floor?.floor_id || 'N/A',
//                 houseDatabaseId: houseDetails?.id
//             };
//         } else if (dataSource === 'manual') {
//             return {
//                 houseId: manualData?.house?.house_name || manualData?.house?.id,
...
//                                 </div>
//                                 <div className="bg-gray-50 dark:bg-gray-900 p-3 md:p-4 rounded-lg">
//                                     <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">Source</p>
//                                     <p className="font-medium text-gray-900 dark:text-white text-sm md:text-base">
//                                         {dataSource === 'qr' ? 'QR Code' : 'Manual'}
//                                     </p>
//                                 </div>
//                             </div>
//                             {dataSource === 'qr' && (
...
//                                         QR Code contained: <code className="bg-blue-100 dark:bg-blue-900 px-2 py-1 rounded">{scannedData?.houseId}</code>
//                                     </p>
//                                 </div>
//                             )}
//                         </div>
...
//                                             <button
//                                                 type="button"
//                                                 onClick={() => navigate(`/measurable-bills/${bill_id}`)}
//                                                 className="w-full bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-300 py-3 px-4 rounded-lg font-medium hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors flex items-center justify-center text-sm md:text-base"
//                                             >
//                                                 <svg className="w-4 h-4 md:w-5 md:h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//                                                     <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
...
//                                         <div className="space-y-2">
//                                             <div className="flex justify-between items-center">
//                                                 <span className="text-gray-600 dark:text-gray-400 text-sm">Bill:</span>
//                                                 <span className="font-medium text-gray-900 dark:text-white text-sm">
//                                                     {billData?.bill_name || 'Measurable Bill'}
//                                                 </span>
//                                             </div>
//                                             <div className="flex justify-between items-center">
...
//                                             </div>
//                                             <div className="flex justify-between items-center">
//                                                 <span className="text-gray-600 dark:text-gray-400 text-sm">House:</span>
//                                                 <span className="font-medium text-gray-900 dark:text-white text-sm">
//                                                     {houseDisplayInfo.houseId}
//                                                 </span>
//                                             </div>
...
//                                                     <div className="flex justify-between items-center">
//                                                         <span className="text-gray-600 dark:text-gray-400 text-sm">Used Units:</span>
...
//                                                 </div>
//                                                 <p className="text-gray-500 dark:text-gray-400 text-sm">
//                                                     Enter current reading to see calculation
//                                                 </p>
//                                             </div>
//                                         )}
...
//                                                 <strong>Note:</strong> QR codes now contain only House ID. System fetches apartment and floor details automatically.
//                                             </p>
//                                         </div>
//                                     </div>
//                                 </div>
...
export default function CalculateMeasurableBill() {
    ...
    // Get data from location state - UPDATED STRUCTURE
    const scannedData = location.state?.scannedData || null;
    ...
    useEffect(() => {
        ...
        if (!scannedData || !houseDetails || !billData) {
            toast.error('House data is incomplete. Please go back and try again.');
            navigate(`/measurable-bills/${bill_id}`);
            return;
        }
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    // Pattern 3: Path parameters with month name
                    try {
                        res = await api.get(`/billprice/${actualBillId}/${monthName}/${formData.year}`);
                    } catch (err3) {
                        apiError = err3;
                        // Pattern 4: Path parameters with month number
                        try {
                            res = await api.get(`/billprice/${actualBillId}/${monthNumber}/${formData.year}`);
                        } catch (err4) {
                            apiError = err4;
                            // Pattern 5: Alternative endpoint
                            try {
    ...
    const loadPreviousReading = async () => {
        ...
        // Get house ID based on source - SIMPLIFIED
        houseId = scannedData?.houseId || scannedData?.hse;
    ...
    // Generate bill handler - SIMPLIFIED
    const handleGenerateBill = async () => {
        if (!calculation) {
            toast.error("Please calculate the bill first");
            return;
        ...
        const houseId = scannedData?.houseId || scannedData?.hse;
        ...
        try {
            ...
            if (response.data.success) {
                ...
                // Navigate back appropriately
                setTimeout(() => {
                    navigate(`/house-bill-meter/${bill_id}`); // Go back to scanner
                }, 2000);
    ...
    useEffect(() => {
        ...
        prevMonthRef.current = formData.month;
        prevYearRef.current = formData.year;
    }, [actualBillId, scannedData]);

    // Load prices when month/year changes or when ranges are loaded
    useEffect(() => {
        if (actualBillId && formData.month && formData.year && billRanges.length > 0) {
            loadAllPricesForMonthYear();
        }
    }, [actualBillId, formData.month, formData.year, billRanges.length]);
                    ...
                    <div className="max-w-7xl mx-auto w-full">
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
                            <div className="flex-1">
                                ...
                                <p className="text-gray-600 dark:text-gray-400 mt-1 text-sm md:text-base">
                                    {source === 'qr' ? 'QR Scanned House' : 'Manually Entered House'}
                                </p>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className={`px-3 py-1 rounded-full text-xs ${source === 'qr' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                                    {source === 'qr' ? '📱 QR Scan' : '✍️ Manual Entry'}
                                </span>
                                <button
                                    onClick={() => {
                                        loadBillRanges();
                            ...
                            {/* <div className="lg:col-span-1">
                                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 md:p-6 lg:sticky lg:top-6">
                                    ...
                                    <div className="space-y-3 md:space-y-4">
                                        ...
                                        <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
                                            <p className="text-xs text-gray-500 dark:text-gray-400">
                                                <strong>Note:</strong> {source === 'qr' ? 'QR codes' : 'Manual input'} contain only House ID. System fetches apartment and floor details automatically.
                                            </p>
                                        </div>
                                    </div>
                                </div>
