...
//     // Get data from location state - UPDATED LOGIC
...
//         // Extract data based on source - UPDATED LOGIC
//         if (dataSource === 'qr') {
//             // QR data structure
//             houseId = scannedData?.house_db_id || scannedData?.house_id;
//             apartmentId = scannedData?.apt_id || scannedData?.apartment_db_id || scannedData?.apartment_id;
//             floorId = scannedData?.f_id || scannedData?.floor_db_id || scannedData?.floor_id;
//         } else if (dataSource === 'manual') {
//             // Manual data structure - FIXED EXTRACTION
//             houseId = manualData?.house?.id || manualData?.house?.house_id;
...
//                 // Optionally navigate back to manual entry or bills list
//                 setTimeout(() => {
//                     if (dataSource === 'manual') {
//                         // Go back to manual entry for next house
//                         navigate(-1);
//                     } else {
//                         // For QR, you might want to go to bills list
//                         // navigate('/measurable-bills');
//                     }
                    
//                     // Reload previous reading for next bill
//                     loadPreviousReading();
//                 }, 2000);
...
// // Add this check at the beginning of your component rendering
// useEffect(() => {
//     if (!bill_id) {
...
//                         {/* House Information Card - UPDATED */}
//                         {(scannedData || manualData) && (
//                             <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 md:p-6 mb-6">
//                                 <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3 md:mb-4">
//                                     House Information ({dataSource === 'qr' ? 'QR Scan' : 'Manual Selection'})
//                                 </h3>
//                                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
//                                     <div className="bg-gray-50 dark:bg-gray-900 p-3 md:p-4 rounded-lg">
//                                         <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">House ID</p>
...
//                                             {dataSource === 'qr' 
//                                                 ? (scannedData.h_id || scannedData.house_id)
//                                                 : (manualData?.house?.house_name || manualData?.house?.id)
//                                             }
//                                         </p>
//                                     </div>
//                                     <div className="bg-gray-50 dark:bg-gray-900 p-3 md:p-4 rounded-lg">
...
//                                             {dataSource === 'qr' 
//                                                 ? (scannedData.apt || scannedData.apartment)
//                                                 : (manualData?.house?.apartment_name || manualData?.house?.apartment_id)
//                                             }
//                                         </p>
//                                     </div>
//                                     <div className="bg-gray-50 dark:bg-gray-900 p-3 md:p-4 rounded-lg">
...
//                                         <p className="font-medium text-gray-900 dark:text-white text-sm md:text-base">
//                                             {dataSource === 'qr' 
//                                                 ? (scannedData.fl || scannedData.floor)
//                                                 : (manualData?.house?.floor_name || manualData?.house?.floor_id)
//                                             }
//                                         </p>
//                                     </div>
//                                 </div>
//                                 <div className="mt-4 flex items-center text-sm">
//                                     <span className={`px-2 py-1 rounded-full text-xs ${dataSource === 'qr' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
//                                         {dataSource === 'qr' ? '📱 QR Scan' : '✍️ Manual Entry'}
//                                     </span>
//                                     {billData && (
//                                         <span className="ml-2 text-gray-600 dark:text-gray-400">
//                                             Bill: {billData.bill_name}
//                                         </span>
//                                     )}
//                                 </div>
//                             </div>
...
export default function CalculateMeasurableBill() {
    ...
    const loadPreviousReading = async () => {
        ...
        if (dataSource === 'qr') {
            houseId = scannedData?.house_db_id || scannedData?.h_id;
        } else if (dataSource === 'manual') {
    ...
    const handleGenerateBill = async () => {
        ...
        if (dataSource === 'qr') {
            // QR data structure
            houseId = scannedData?.house_db_id || scannedData?.h_id;
            apartmentId = scannedData?.apt_id || scannedData?.apartment_db_id;
            floorId = scannedData?.f_id || scannedData?.floor_db_id;
        } else if (dataSource === 'manual') {
            ...
            if (response.data.success) {
                ...
                setTimeout(() => {
                    if (dataSource === 'manual') {
                        ...
                        navigate('/measurable-bills'); // Go to QR scanner home
                    }
                }, 2000);
    ...
    const getHouseDisplayInfo = () => {
        if (dataSource === 'qr') {
            return {
                houseId: scannedData?.h_id || scannedData?.house_id,
                apartment: scannedData?.apt || scannedData?.apartment,
                floor: scannedData?.fl || scannedData?.floor
            };
        } else if (dataSource === 'manual') {
            return {
