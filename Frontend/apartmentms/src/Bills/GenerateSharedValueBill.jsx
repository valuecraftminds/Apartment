...
//     if (roundTo === 'ten') {
//         // Round up to nearest 10 (5670, 4910, etc.)
//         return Math.ceil(number / 10) * 10;
//     } else if (roundTo === 'hundred') {
//         // Round up to nearest 100
//         return Math.ceil(number / 100) * 100;
//     } else {
//         // Standard round up to specified decimal places
...
//                 const estimatedUnitPrice = parseFloat(amount) / houses.length;
//                 const roundedUnitPrice = roundUpCustom(estimatedUnitPrice, 2, 'ten');
//                 setUnitPrice(roundedUnitPrice); // Will be calculated on the server
...
//             // Round UP to nearest 10 (as per client requirement)
//             const roundedUnitPrice = roundUpCustom(calculatedUnitPrice, 2, 'ten');
            
//             // Or use standard rounding up to 2 decimal places:
//             // const roundedUnitPrice = roundUpToNext(calculatedUnitPrice, 2);
            
//             setUnitPrice(roundedUnitPrice);
//             }
//         } else {
//             setUnitPrice(0);
//         }
...
//                         min="2025"
//                         max="2100"
//                         className="w-full px-3 py-2 text-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:text-white"
...
export default function GenerateSharedValueBill({ onClose, onCreated,apartment_id }) {
    ...
    const handleTotalAmountChange = (amount) => {
        ...
        // Always use all assigned houses
...
//     const count = assignedHousesCount;

//     if (count === 0) {
//         setError('No houses are assigned to this bill for the selected apartment.');
//         return;
//     }
...
//         if (calculationMethod === 'house_count') {
//             // For apartment-level house count generation, use the original endpoint
//             const singleBillData = {
//                 bill_id: selectedBill,
//                 year: year,
...
//             response = await api.post('/generate-bills', singleBillData);
//         } else {
//             // Use multiple bills generation for square footage calculation
//             response = await api.post('/generate-bills/multiple', billData);
//         }
...
const handleGenerateBill = async () => {
    ...
    try {
        ...
        // ALWAYS use multiple bills endpoint for generating bills for all assigned houses
        const billData = {
            bill_id: selectedBill,
            year: year,
            month: month,
            totalAmount: parseFloat(totalAmount),
            apartment_id: selectedApartment,
            // Send empty arrays since we're using ALL assigned houses
            selected_floors: [],
            selected_houses: [],
            calculation_method: calculationMethod,
            due_date: dueDate || null
       ...
       // console.log('Sending data to /generate-bills/multiple:', billData);
        const response = await api.post('/generate-bills/multiple', billData);
        
      //  console.log('Server response:', response.data);
        
        if (response.data.success) {
            toast.success(`Successfully generated ${response.data.data?.length || 0} bills!`);
            onCreated(response.data.data);
        } else {
            setError(response.data.message || 'Failed to generate bill');
        }
        ...
        let errorMsg = 'Failed to generate bill. Please try again.';
    ...
    return (
        <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 pl-1">
            ...
            {/* Assigned Houses Information */}
            {selectedBill && selectedApartment && (
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span className="text-gray-600 dark:text-gray-400">Total Assigned Houses:</span>
                            <p className="font-medium text-gray-800 dark:text-white">
                                {assignedHousesCount}
                            </p>
                        </div>
                    ...
                    {/* Show assigned houses count by floor */}
                    {floorsData.length > 0 && (
                        <div className="mt-3">
                            ...
                            <div className="space-y-1 max-h-32 overflow-y-auto pr-2">
                                {floorsData.map(floor => (
                                    <div key={floor.id} className="flex justify-between text-xs">
                                        <span className="text-gray-600 dark:text-gray-400">Floor {floor.floor_id}:</span>
                                        <span className="font-medium text-gray-700 dark:text-gray-300">
                                            {floor.assigned_houses_count} houses
                                        </span>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <p className="font-medium text-gray-800 dark:text-white">{assignedHousesCount}</p>
                        </div>
                    </div>
                    ...
                    {totalAmount && assignedHousesCount > 0 && (
                        <div className="border-t border-gray-200 dark:border-gray-600 pt-3">
                            ...
                            {calculationMethod === 'house_count' && (
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                                    Calculation: ${totalAmount} ÷ {assignedHousesCount} houses = 
                                    <span className="font-medium"> ${(parseFloat(totalAmount) / assignedHousesCount).toFixed(6)}</span> 
                                    <span className="mx-2">→</span>
                                    Rounded up to: <span className="font-medium">${unitPrice.toFixed(2)}</span> per house
                                </p>
                            )}
                ...
                <div className="flex justify-end space-x-3">
                    ...
                    >
                        {generating ? (
                            ...
                            <span>
                                Generate {assignedHousesCount} Bills
                            </span>
                        )}
                    </button>
                </div>
