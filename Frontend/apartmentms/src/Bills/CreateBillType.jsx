...
export default function CreateBillType({ onClose, onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation
    if (!formData.bill_name.trim() || !formData.billtype) {
    ...
    try {
      ...
      if (result.data.success) {
        ...
        setError(result.data.message || "Failed to save bill type");
      }
    } catch (err) {
      console.error("Error saving bill type:", err);
      const errorMessage = err.response?.data?.message || "Error saving bill type. Please try again.";
      setError(errorMessage);
      
      // Show specific message for duplicate bills
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex justify-end gap-3 pt-4">
        ...
        <button
          type="submit"
          disabled={loading || !formData.bill_name.trim() || !formData.billtype}
          className={`px-4 py-2 rounded-lg text-white transition-colors font-medium ${
            loading || !formData.bill_name.trim() || !formData.billtype
